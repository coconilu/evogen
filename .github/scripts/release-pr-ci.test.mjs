...
const pending = {
  ...
  repository: repo,
  pull_requests: [{ number: pr.number, head: { sha }, base: { ref: 'main' } }],
  status: 'completed',
...
test('真实回归：dispatch 成功仍有待批准 PR suite，批准一次并等 PR CI 才放行', async () => {
  ...
  assert.deepEqual(writes, [{ path: '/actions/runs/200/approve', method: 'POST' }]);
});

test('不批准无关 PR、旧提交、其他分支 / 工作流或其他仓库', () => {
  for (const patch of [
    { pull_requests: [{ number: 12, head: { sha }, base: { ref: 'main' } }] },
...
test('已经运行或通过的 PR CI 不重复审批；失败、权限拒绝和超时不放行', async () => {
  ...
  for (const conclusion of ['failure', 'cancelled', 'skipped']) {
    await assert.rejects(
      runPullRequestCI(async () => ({ workflow_runs: [{ ...pending, conclusion }] }), pr, { log: () => {} }),
      /未通过/,
    );
  }
  await assert.rejects(
...
test('GitHub 审批 201 / dispatch 204 空响应正确处理，405 保留具体阻塞原因', async () => {
  for (const status of [201, 202, 204]) {
    const api = createGitHubAPI('owner/repo', 'test-token', async () => new Response(null, { status }));
    assert.equal(await api('/actions/runs/200/approve', { method: 'POST' }), null);
  }
  ...
  await assert.rejects(
    ...
    (error) => error.status === 405 && error.message.includes('Required status check "check" is expected.'),
  );
});
