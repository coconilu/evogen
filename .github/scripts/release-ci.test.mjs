...
import { completeVersionRelease, dispatchedRun, mergeVersionPR, runCI } from './release-ci.mjs';
...
test('只认本次 dispatch 后正确提交、分支和 CI 工作流的运行', () => {
  const otherRuns = [
    ...
    { ...run, id: 13, head_branch: 'main' },
    { ...run, id: 14, event: 'pull_request' },
    { ...run, id: 15, path: '.github/workflows/other.yml' },
  ];
  assert.equal(dispatchedRun(otherRuns, { afterId: 10, sha, branch }), undefined);
  assert.equal(dispatchedRun([...otherRuns, run], { afterId: 10, sha, branch }), run);
});
    ...
    await assert.rejects(
      runCI(
        async (_endpoint, options) =>
          options ? null : { workflow_runs: ++reads === 1 ? [] : [{ ...run, conclusion }] },
        branch,
        sha,
        { log: () => {} },
      ),
...
test('等待不到目标提交会超时，不使用另一提交的成功检查', async () => {
  ...
  await assert.rejects(
    runCI(
      async (_endpoint, options) => (options ? null : { workflow_runs: [{ ...run, head_sha: mergedSha }] }),
      branch,
      sha,
      {
        now: () => time,
...
test('版本 PR CI → 按 head 合并 → 合并提交 CI → 返回同一发布 SHA', async () => {
  ...
  const result = await completeVersionRelease(pr, {
    checkPR: async (candidate) => calls.push(['pr-check', candidate.number, candidate.head.sha]),
  ...
  assert.deepEqual(calls, [
    ['pr-check', 3, sha],
    ['merge', 3, sha],
    ['current', 'main', mergedSha],
    ['check', 'main', mergedSha],
  ]);
});
...
test('同一运行重试复用已合并 PR，不重复合并；已关闭未合并则停止', async () => {
  ...
  assert.deepEqual(calls, [['main', mergedSha]]);
  await assert.rejects(completeVersionRelease({ ...pr, state: 'closed' }, handlers), /已关闭/);
});
...
test('CI 状态传播延迟时有限重试合并，每次都固定 PR head', async () => {
  ...
  const api = async (_endpoint, options) => {
    ...
    if (++puts === 1) throw Object.assign(new Error('checks propagating'), { status: 405 });
    return { merged: true, sha: 'c'.repeat(40) };
  };
  assert.equal(
    await mergeVersionPR(api, 3, mergedSha, 'main', sha, {
      sleep: async () => {},
    }),
    'c'.repeat(40),
  );
...
test('手动 patch/minor/major 必须发布自动合并并通过校验的 SHA', () => {
  ...
  for (const bump of ['patch', 'minor', 'major']) {
    assert.equal(
      releaseTarget('workflow_dispatch', { repository, inputs: { bump } }, sha, 'refs/heads/main', mergedSha)
        .sha,
      mergedSha,
    );
    assert.equal(
      releaseTarget('workflow_dispatch', { repository, inputs: { bump } }, sha, 'refs/heads/main'),
      null,
    );
  }
  assert.throws(() =>
...
test('重试只能复用预期版本文件，拒绝夹带源码、依赖脚本或篡改版本', () => {
  const expected = {
    'apps/desktop/package.json': '{"version":"1.0.1","scripts":{"build":"tsc"}}',
  ...
  assert.doesNotThrow(() =>
    assertVersionFiles(
      ...
      {
        ...expected,
        'CHANGELOG.md': expected['CHANGELOG.md'].replace('2026-09-07', '2026-09-08'),
      },
      expected,
    ),
  );
  assert.throws(
    () => assertVersionFiles([...Object.keys(expected), 'apps/desktop/src/App.tsx'], expected, expected),
    /版本文件以外/,
  );
  assert.throws(
    () =>
      assertVersionFiles(
        ...
        {
          ...expected,
          'apps/desktop/package.json': '{"version":"1.0.1","scripts":{"build":"malicious"}}',
        },
        expected,
      ),
    /不一致/,
  ...
  assert.throws(
    () =>
      assertVersionFiles(
        Object.keys(expected),
        { ...expected, 'apps/desktop/src-tauri/tauri.conf.json': '{"version":"2.0.0"}' },
        expected,
      ),
    /不一致/,
  );
...
test('快速通道判定与 version job 同一标准：node 与 desktop 最新 check 均成功才算绿', async () => {
  const green = (name, id) => ({
    id,
    name,
    app: { id: 15368 },
  ...
  assert.equal(
    await mainHasGreenCI(
      async () => ({
        check_runs: [green('node', 1), green('desktop', 2), { ...green('node', 3), conclusion: 'failure' }],
      }),
      sha,
    ),
    false,
  ...
  assert.equal(
    await mainHasGreenCI(async () => ({ check_runs: [green('node', 1), green('desktop', 2)] }), sha),
    true,
  );
  assert.equal(await mainHasGreenCI(async () => ({ check_runs: [green('node', 1)] }), sha), false);
  assert.equal(await mainHasGreenCI(async () => ({}), sha), false);
});

test('快速通道本地校验：版本文件可解析且版本一致', () => {
  assert.doesNotThrow(() =>
    assertFastTrackVersionFiles(JSON.stringify({ version: '1.2.5' }), JSON.stringify({ version: '1.2.5' })),
  );
  assert.throws(
    () => assertFastTrackVersionFiles('{oops', JSON.stringify({ version: '1.2.5' })),
    /package\.json 不是有效的 JSON/,
  );
  assert.throws(
    () => assertFastTrackVersionFiles(JSON.stringify({ version: '1.2.5' }), '{oops'),
    /tauri\.conf\.json 不是有效的 JSON/,
  );
  assert.throws(
    () =>
      assertFastTrackVersionFiles(JSON.stringify({ version: '1.2.5' }), JSON.stringify({ version: '1.3.0' })),
    /版本不一致/,
  );
});
