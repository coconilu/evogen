...
const repo = process.env.GITHUB_REPOSITORY ?? 'coconilu/evogen';
...
function findSignedPair(dir, suffix) {
  ...
  const file = files.find((f) => f.endsWith(suffix) && !f.endsWith(`${suffix}.sig`));
  if (!file) {
    console.error(`${dir} 目录存在但找不到 ${suffix} 安装包（构建或上传步骤异常？）`);
    process.exit(1);
  }
...
for (const [dir, suffix, platform] of targets) {
  ...
  if (!pair) {
    console.error(`缺少 ${platform} 平台产物（${artifactsDir}/${dir}/ 不存在）`);
    process.exit(1);
  }
  platforms[platform] = {
