...
export function pullRequestRun(runs, pr) {
  ...
  if (!repo || pr.head.repo?.full_name !== repo || !/^codex\/release-\d+$/.test(pr.head.ref))
    ...
    .filter(
      (run) =>
        ...
        run.pull_requests?.some(
          (linked) =>
            linked.number === pr.number && linked.head.sha === pr.head.sha && linked.base.ref === pr.base.ref,
        ),
    )
...
export async function runPullRequestCI(
  ...
  while (now() < deadline) {
    ...
    if (run?.conclusion === 'action_required') {
      ...
      if (run.conclusion !== 'success') throw new Error('版本 PR CI 未通过，发布已停止: ' + run.html_url);
      return run;
    }
    await sleep(10_000);
  }
...
export async function runCI(
  ...
  while (now() < deadline) {
    ...
    if (run?.status === 'completed') {
      if (run.conclusion !== 'success') throw new Error('CI 未通过，发布已停止: ' + run.html_url);
      return run;
    }
    await sleep(10_000);
  }
  throw new Error('等待提交 ' + sha + ' 的 CI 超时，发布已停止；可重跑失败任务。');
}

// Both the version PR and the resulting merge commit need their own CI evidence.
export async function completeVersionRelease(pr, { checkPR, check, merge, assertCurrent }) {
  if (pr.state !== 'open' && !pr.merged) throw new Error('版本 PR 已关闭且未合并，停止发布');
  let sha = pr.merge_commit_sha;
  if (!pr.merged) {
    await checkPR(pr);
    sha = await merge(pr.number, pr.head.sha);
  }
  if (!/^[a-f0-9]{40}$/.test(sha ?? '')) throw new Error('缺少已合并的版本提交，停止发布');
...
export async function mergeVersionPR(
  ...
  base,
  baseSha,
  { sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms)), attempts = 12 } = {},
) {
  for (let attempt = 0; attempt < attempts; attempt++) {
    const current = await api('/git/ref/heads/' + base);
    if (current.object.sha !== baseSha)
      throw new Error('主分支在 CI 期间发生变化，停止自动合并；请从当前主分支重跑。');
    try {
