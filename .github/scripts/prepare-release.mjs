...
import { completeVersionRelease, mergeVersionPR, runCI, runPullRequestCI } from './release-ci.mjs';
import { createGitHubAPI } from './release-github.mjs';
import { hasRequiredCheck } from './release-target.mjs';

const VERSION_FILES = ['apps/desktop/package.json', 'apps/desktop/src-tauri/tauri.conf.json', 'CHANGELOG.md'];
...
// 快速通道前提：main HEAD 已有绿 CI。与 version job 校验合并提交使用同一个
// hasRequiredCheck 标准（GitHub Actions 产出的最新 check 必须 success）。
export async function mainHasGreenCI(api, sha) {
  const { check_runs: checkRuns } = await api('/commits/' + sha + '/check-runs?filter=latest&per_page=100');
  return hasRequiredCheck(checkRuns ?? []);
}

// 快速通道本地校验：版本 PR 只改版本文件且 main 已有绿 CI 时，
...
export function assertFastTrackVersionFiles(pkgText, tauriText) {
  ...
  try {
    ...
    throw new Error('apps/desktop/package.json 不是有效的 JSON: ' + error.message);
  }
  try {
    tauri = JSON.parse(tauriText);
  } catch (error) {
    throw new Error('apps/desktop/src-tauri/tauri.conf.json 不是有效的 JSON: ' + error.message);
  }
  if (pkg.version !== tauri.version)
    throw new Error('package.json 与 tauri.conf.json 版本不一致: ' + pkg.version + ' != ' + tauri.version);
}

// 快速通道合并：分支保护若要求版本 PR 自身的 check，回退等待该 PR 的 CI 完成后再合并。
export function mergeWithPRCIFallback({ merge, waitForPRCI }) {
  return async (...args) => {
    try {
      return await merge(...args);
    } catch (error) {
      if (error?.status !== 405 || !/required status check/i.test(error.message ?? '')) throw error;
      await waitForPRCI();
      return merge(...args);
    }
  };
...
async function main() {
  ...
  if (process.env.GITHUB_REF !== 'refs/heads/' + branch) throw new Error('请从默认分支运行发布流程');
  ...
  if (baseSha !== process.env.GITHUB_SHA) throw new Error('准备版本的源码不是本次触发的提交');
  // 快速通道判定：main HEAD 已有绿 CI 时，版本 PR 只改版本文件、源码与主分支
  // 完全一致，可跳过完整 CI 等待（见下方 checkPR / merge 的 fastTrack 分支）。
  const fastTrack = await mainHasGreenCI(api, baseSha);
  summary(
    fastTrack
      ? 'main HEAD 已有绿 CI，版本 PR 走快速通道：本地校验版本文件后按分支保护合并'
      : 'main HEAD 尚无绿 CI，版本 PR 等待完整 CI 后合并',
  );
  ...
  const expected = Object.fromEntries(VERSION_FILES.map((file) => [file, readFileSync(file, 'utf8')]));
  const prs = await api('/pulls?state=all&head=' + encodeURIComponent(repo.split('/')[0] + ':' + head));
  let pr = prs[0] ? await api('/pulls/' + prs[0].number) : null;
  const remoteHead = await api('/git/ref/heads/' + head, { missing: true });
  const existingSha = remoteHead?.object.sha ?? (pr?.merged ? pr.merge_commit_sha : null);
  if (existingSha) {
    run('git', ['fetch', 'origin', 'refs/heads/' + (remoteHead ? head : branch)]);
    ...
    const files = run('git', ['diff', '--name-only', baseSha, candidate]).split('\n').filter(Boolean);
    const actual = Object.fromEntries(
      VERSION_FILES.map((file) => [file, run('git', ['show', candidate + ':' + file])]),
    );
  ...
  if (pr.base.ref !== branch || pr.head.ref !== head || pr.head.repo?.full_name !== repo)
    throw new Error('版本 PR 的仓库或分支不匹配，停止发布');
  const verifiedHead = remoteHead?.object.sha ?? run('git', ['rev-parse', 'HEAD']);
  if (!pr.merged && pr.head.sha !== verifiedHead) throw new Error('版本 PR 的提交发生变化，停止发布');
  summary('版本 PR（自动处理）: ' + pr.html_url);
  const sha = await completeVersionRelease(pr, {
    checkPR: fastTrack
      ? async () => {
          const pkgText = readFileSync('apps/desktop/package.json', 'utf8');
          const tauriText = readFileSync('apps/desktop/src-tauri/tauri.conf.json', 'utf8');
          assertFastTrackVersionFiles(pkgText, tauriText);
          // biome 声明在仓库根 package.json；固定用仓库声明的版本，
          // 与 CI 的 pnpm lint（biome ci .）同一标准
          const root = JSON.parse(readFileSync('package.json', 'utf8'));
          const biome = root.devDependencies?.['@biomejs/biome'];
          if (!biome) throw new Error('根 package.json 未声明 @biomejs/biome 版本');
          execFileSync(
            'npx',
            [
              '--yes',
    ...
    assertCurrent: async (ref, commit) => {
      ...
      if (current.object.sha !== commit)
        throw new Error('主分支已发生变化，停止本次发布；可在当前主分支重新运行。');
    },
    merge: fastTrack
      ? mergeWithPRCIFallback({
          merge: (number, commit) => mergeVersionPR(api, number, commit, branch, baseSha),
          waitForPRCI: () => {
            summary('分支保护要求版本 PR 自身的 check，回退等待该 CI 完成后合并');
            return runPullRequestCI(api, pr);
          },
        })
      : (number, commit) => mergeVersionPR(api, number, commit, branch, baseSha),
  });
  appendFileSync(process.env.GITHUB_OUTPUT, 'sha=' + sha + '\n');
  summary('版本 v' + version + ' 已自动合并并通过 CI，继续构建与发布提交 ' + sha + '。');
}
