...
export function releaseTarget(eventName, event, sha, ref, preparedSha) {
  ...
  if (eventName === 'workflow_dispatch') {
    if (ref !== 'refs/heads/' + branch) throw new Error('请从默认分支运行发布流程');
    if (event.inputs.bump !== 'none') {
      if (!['patch', 'minor', 'major'].includes(event.inputs.bump)) throw new Error('未知的版本增量');
...
// evogen 的必需检查是 CI workflow 的两个 job：node 与 desktop。
// 只认 GitHub Actions（app id 15368）产出的最新一次运行，每个名字取 id 最大的。
export function hasRequiredCheck(checks) {
  ...
  for (const name of ['node', 'desktop']) {
...
async function main() {
  ...
  execFileSync('git', ['merge-base', '--is-ancestor', target.sha, 'origin/' + branch]);
  const readAt = (file) =>
    JSON.parse(
  ...
  const tag = releaseTag(
    readAt('apps/desktop/package.json').version,
    readAt('apps/desktop/src-tauri/tauri.conf.json').version,
  );
  ...
  const checks = await github('/commits/' + target.sha + '/check-runs?filter=latest&per_page=100');
  if (!hasRequiredCheck(checks.check_runs))
    throw new Error('提交 ' + target.sha + ' 尚未通过 node/desktop 检查，请先在该提交运行 CI');
